    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                logger.info(f'[VAL] Begin validation at iter {current_step}')
                avg_psnr = 0.0
                ...
                logger.info(f'[VAL] End validation at iter {current_step}')
            ...
            if current_step % opt['logger']['save_checkpoint_freq'] == 0:
                if rank <= 0:
                    logger.info(f'[SAVE] Begin saving models at iter {current_step}')
                    model.save(current_step)
                    model.save_training_state(epoch, current_step)
                    logger.info(f'[SAVE] End saving models at iter {current_step}')
